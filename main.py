...
async def reject_spam_loop(target_uid, key, iv):
    ...
    while reject_spam_running and count < max_spam:
        try:
            packet1 = await SPIDY1(target_uid, key, iv)
            packet2 = await SPIDY(target_uid, key, iv)
...
async def SPIDY(client_id, key, iv):
    banner_text = f""" 
.
.
.
...
[CCFFCC]━━━━━━━━━━━━━━━━━━━━━━━━━━
[B][C][81DACA]P O W E R E D   B Y S P I D Y
[FFFF00]━━━━━━━━━━━━━━━━━━━━━━━━━━
[B][C][00FF00]F O L L O W   M E   O N   I N S T A G R A M
[B][C][FFFFFF]@mad_psyc._
[00008B]━━━━━━━━━━━━━━━━━━━━━━━━━━
...
async def SPIDY1(client_id, key, iv):
    gay_text = f""" 
.
.
.
...
    
[B][C][00FF00]★ D E V   S P I D Y  C O D E X ★
[FF0000]━━━━━━━━━━━━━━━━━━━━━━━━━━
[B][C][FF9900]D O N E   H A C K I N G
[B][C][E75480]Y O U R   A C C O U N T
[81DACA]━━━━━━━━━━━━━━━━━━━━━━━━━━
...
async def convert_kyro_to_your_system(target_uid, chat_id, key, iv, nickname="SPIDY", title_id=904990072):
    try:
        fields = {
            1: 1,
            2: {
...
async def send_kyro_title_adapted(chat_id, key, iv, target_uid, nickname="SPIDY", title_id=905190079):
    try:
...
async def send_all_titles_once(chat_id, key, iv, target_uid, whisper_writer, nickname="SPIDY"):
    titles = [
...
async def handle_title_final(inPuTMsG, uid, chat_id, key, iv, region, chat_type=0):
    ...
    try:
        if not whisper_writer:
            return
        title_packet = await convert_kyro_to_your_system(target_uid, chat_id, key, iv, "SPIDY", selected_title)
        if title_packet and whisper_writer:
            whisper_writer.write(title_packet)
            await whisper_writer.drain()
    except Exception:
...
async def send_sticker(target_uid, chat_id, key, iv, nickname="SPIDYxBOT"):
    try:
            ...
            while True:
                ...
                if data_hex.startswith("0500") and insquad is None and joining_team == False:
                    try:
                        ...
                        insquad = True
                        try:
                            welcome_message = """[B][C][FFFFFF]Welcome to SURYA's Bot
...
[B][C][FFFFFF]Telegram : @surya911zz
[B][C][FFFFFF]Contact : @mad_psyc._
...
[B][C][FFFFFF]Status: Online & Active"""
                ...
                if data2.hex().startswith('0500') and len(data2.hex()) > 1000:
                    try:
                        ...
                        message = """[B][C][FFFFFF]Welcome to SPIDY Bot
...
[B][C][FFFFFF]Telegram : @shahnilraza
[B][C][FFFFFF]Contact : @Spideryt_1m
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/room'):
                            ...
                            if len(parts) < 3:
                                ...
                                await dl_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                try:
                                    success = await Room_Spam(target_uid, room_id, "SPIDY CODEX", key, iv)
                                    if success:
                        ...
                        if inPuTMsG.strip().startswith('/sticker'):
                            packet = await send_sticker(uid, chat_id, key, iv, nickname="SPIDYxBOT")
                            if packet:
...
"""
                        ...
                        if inPuTMsG.strip() == '/stop evo_c':
                            if evo_custom_spam_task and not evo_custom_spam_task.done():
                                ...
                                await dl_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        if inPuTMsG.strip().lower() == "SPIDY bhai mera uid do":
...
async def MaiiiinE():
    ...
    Uid, Pw = '4979804439', 'BBED89930D1E2D5BB4B31A65BE9C74EFD4D69A878EF9847BD5EA9306AC74BC3D'
    ...
    os.system('clear')
    print(render('SURYA', colors=['white', 'green'], align='center'))
    print('')
    print("🤖 SURYA's BOT - ONLINE")
    print("=" * 50)
    print(f"🔹 UID: {TarGeT}")
